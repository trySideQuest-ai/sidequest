...
# polls for completion (with 10+ hour timeout due to Apple service delays),
...
#   ./macOS/scripts/notarize.sh build/Release/SideQuest.dmg --timeout 10h
...
if [ -z "$APP_PATH" ] || [ "$APP_PATH" = "." ]; then
  log_error "Usage: $0 <path-to-.app-or-.dmg> [--timeout 10h]"
  exit 1
fi
...
log_info "Apple is currently experiencing service delays (10+ hours typical wait time)"
...
REQUEST_ID=$(echo "$SUBMIT_OUTPUT" | grep -i "id:" | awk '{print $NF}' | head -1 || true)
...
if echo "$SUBMIT_OUTPUT" | grep -qi "Approved"; then
  ...
  # Status unclear; might still be in progress if timeout occurred
  log_error "Could not determine notarization status"
  echo ""
  ...
  echo ""
  echo "Once approved, staple the ticket manually:"
...
echo ""
log_info "Next steps:"
echo "  1. Create DMG: ./macOS/scripts/create-dmg.sh \"$APP_PATH\""
echo "  2. Optionally sign DMG: ./macOS/scripts/sign-dmg.sh build/Release/SideQuest.dmg"
echo "  3. Test on fresh Mac before distributing to users"
echo ""
